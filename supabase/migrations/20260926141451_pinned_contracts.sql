-- Personal contract pins use the application's opaque app user sessions.
create table public.pinned_contracts (
  user_id uuid not null references public.app_users(id) on delete cascade,
  contract_id text not null references public.contrat(id_contrat) on delete cascade,
  created_at timestamptz not null default now(),
  primary key (user_id, contract_id)
);

alter table public.pinned_contracts enable row level security;
revoke all on table public.pinned_contracts from public, anon, authenticated;

create function public.read_pinned_contracts(p_session_token text)
...
create function public.set_pinned_contract(p_session_token text, p_contract_id text, p_pinned boolean)
returns text[] language plpgsql security definer set search_path = '' as $$
declare
  actor_id uuid;
...
begin
  ...
  -- Serialize additions so two devices cannot exceed the ten-contract limit.
  ...
  return public.read_pinned_contracts(p_session_token);
end;
$$;

revoke all on function public.read_pinned_contracts(text) from public;
revoke all on function public.set_pinned_contract(text,text,boolean) from public;
grant execute on function public.read_pinned_contracts(text) to anon, authenticated;
grant execute on function public.set_pinned_contract(text,text,boolean) to anon, authenticated;
