...
for row in range(6,54):
 ...
 for raw in re.split(r',\s*|\s+/\s+',titles):
  ...
  if key not in entries:
   entries[key]={'id':f'grid-2022-{row}-{len(entries)+1}','masculine':label,'feminine':feminine(label),'category':category,'salaries':[],'aliases':[],'effectiveDate':'2022-04-01','source':'Grille salariale Haïti — mai 2022','sourceRows':[],'notes':'','active':True,'version':1}
  e=entries[key]
...
for row,male,cat in [(6,'Magasinier','Personnel de soutien'),(7,'Régisseur de pharmacie','Personnel médical'),(8,'Archiviste','Personnel administratif'),(9,'Massothérapeute','Personnel médical'),(10,'Manutentionnaire','Personnel de soutien')]:
 ...
 entries[('annotation',male)]={'id':f'grid-note-{row}','masculine':male,'feminine':feminine(male),'category':cat,'salaries':[salary],'aliases':[raw] if raw!=male else [],'effectiveDate':None,'source':'Annotations manuscrites — sans date','sourceRows':[row],'notes':note or 'Montant manuscrit non daté, à valider.','active':False,'version':1}
